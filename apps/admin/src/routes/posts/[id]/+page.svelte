        ...
        if (data.languages && Object.keys(translationsData).length === 0) {
            ...
            data.languages.forEach((lang: any) => {
                ...
                if (existingPost?.tags) {
                    try {
                        const parsed = JSON.parse(existingPost.tags);
                        if (Array.isArray(parsed)) {
                            parsedTags = parsed.join(", ");
                        } else {
                            parsedTags = String(existingPost.tags);
                        }
                    } catch (e) {
                        parsedTags = String(existingPost.tags);
                    }
                ...
                const finalSlug = existingPost?.slug || (basePost ? basePost.slug : "");
                const finalCategory = existingPost?.category_slug || existingPost?.category || (basePost ? (basePost.category_slug || basePost.category) : "");
                let finalCategoryName = existingPost?.category_name || existingPost?.categoryName || "";

                if (!existingPost && finalCategory) {
                    const match = data.categories.find((c: any) => c.slug === finalCategory && c.lang === lang.code);
                    if (match) {
                ...
                translationsData[lang.code] = {
                    id: existingPost?.id || 'new-' + lang.code,
                    ...
                    authorId: existingPost?.author_id || "",
...
---`;
        ...
        // Set the hidden field with the entire configured group data
...
<div class="write-container">
    <div class="header">
        <h1>{t('admin.posts.write.edit_title', { default: '글 수정 (다국어 묶음)' })}</h1>
        ...
        <div class="flex gap-2">
            {#each data.languages || [] as lang}
                ...
                >
                    ...
                    {lang.name}
                    {#if translationsData[lang.code]?.id?.startsWith('new-')}
