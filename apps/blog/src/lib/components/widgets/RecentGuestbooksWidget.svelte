...
<div class="recent-guestbooks-widget">
    {#if guestbooks.length === 0}
        <div class="widget-empty">{$t("admin.dashboard.recent.empty_guestbook", { default: "최근 등록된 방명록이 없습니다." })}</div>
    {:else}
            ...
            {#each guestbooks as gb}
                <li class="widget-item">
                    <a href={`/${lang}/guestbook`} class="widget-link">
                        ...
                        {:else}
                            {$t("blog.widget.recent_guestbooks_format", { author_name: gb.user_name })}
                        {/if}
                    </a>
                </li>
            {/each}
